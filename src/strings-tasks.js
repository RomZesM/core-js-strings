...
function isString(value) {
  ...
  if (typeof value === 'object') {
    if (value.constructor === String) return true;
  }
  return false;
}
/**
...
function reverseWords(sent) {
  ...
  const retArr = [];
  words.forEach((element) => {
    retArr.push(reverseStr(element));
  });
  return retArr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (
      (asciiCode > 64 && asciiCode < 78) ||
      (asciiCode > 96 && asciiCode < 110)
    ) {
      ret = ret.concat(String.fromCharCode(asciiCode + 13));
    } else if (
      (asciiCode > 77 && asciiCode < 91) ||
      (asciiCode > 109 && asciiCode < 123)
    ) {
      ret = ret.concat(String.fromCharCode(asciiCode - 13));
    } else ret = ret.concat(str[i]);
  }
...
function getCardId(card) {
  let cardIndex = Number.parseInt(card[0], 10);
  let cardPow = 0;
  let id = 0;
  if (card.length === 2) {
